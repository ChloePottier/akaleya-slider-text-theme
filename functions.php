...
function cpt_slider_init(){
    $labels = array(
        ...
        'edit_item'             => __('Modifier le slide', 'textdomain'),
...
function cpt_prestations_init(){
    $labels = array(
        ...
        'edit_item'             => __('Modifier la prestation', 'textdomain'),
...
function prestations_loop_shortcode($atts){
    ...
    $loop_tem = new  WP_Query(array('post_type' => 'prestation','posts_per_page' => $perpage,
    'post_parent'=> 0, 'orderby'=> 'menu_order', 'order'=>'ASC'
     ));
    while ($loop_tem->have_posts()) : $loop_tem->the_post();
        $output .= '<a href="'.get_permalink().'" class="col-12 col-md-6 col-lg-4 pb-5"><div class="prestations-items"> '.
        get_the_post_thumbnail('', 'prestations', array( 'class' => 'responsive' )).
        '<h4 class="mt-3 pb-3">' . get_the_title() . '</h4>';
        if(has_excerpt()):
